...
# 创建文件夹
if not os.path.exists('./image/number'):
    os.mkdir('./image/number')
...
for epoch in range(num_epoch):  # 进行多个epoch的训练
    for i, (img, _) in enumerate(dataloader):
        ...
        if epoch == 0:
            ...
            save_image(real_images, './image/number/real_images.png')
    fake_images = to_img(fake_img.cpu().data)
    save_image(fake_images, './image/number/fake_images-{}.png'.format(epoch + 1))
